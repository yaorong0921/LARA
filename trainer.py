...
class trainer(object):
    def __init__(self, args):
        ...
        if not args.exp_name:
            self.saved_name = f"{args.type_model}_{args.dataset}.pth"
        else:
            self.saved_name = f"{args.exp_name}.pth"
    def load_trained_model(self):
        savepath = os.path.join('./saved_models', self.saved_name)
    ...
    def train_and_test(self, seed):
        ...
        for epoch in range(self.epochs):
            ...
            if epoch % self.eval_steps == 0 and epoch != 0:
                ...
                results_tmp = 100 * np.array(results)
                best_idx = np.argmax(results_tmp[:, 1])
                best_valid = results_tmp[best_idx, 1]
                state = {
                    'epoch': epoch,
                    'state_dict': self.model.state_dict(),
                    'optimizer': self.optimizer.state_dict(),
                    }
                if best_valid > best_acc:
                    best_acc = best_valid
